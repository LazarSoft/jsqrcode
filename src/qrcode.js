...
{
  var canvas_qr, context;

  if(arguments.length == 1 && typeof src === 'object')
  {
    if(src.tagName === 'CANVAS')
    {
      canvas_qr = src;
      src = canvas_qr.getContext('2d');
    }

    if(src instanceof CanvasRenderingContext2D)
    {
      context = src;
      canvas_qr = context.canvas;
    }
  }
  ...
  for (var ay = 0; ay < sqrtNumArea; ay++)
  {
    for (var ax = 0; ax < sqrtNumArea; ax++)
    {
      for (var dy = 0; dy < areaHeight; dy++)
      {
        for (var dx = 0; dx < areaWidth; dx++)
        {
          bitmap[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] = grayScale[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] < middle[ax][ay];
        }
      }
    }
  }
